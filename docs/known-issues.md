...
**근본 해결 (대기)**
- Replicate BRIA RMBG 2.0 도입 — 진짜 픽셀 마스크 기반, $0.018/장
- 데모 기간 반응 보고 결정
...
| 에러 핸들링 중복 | 각 API 라우트마다 try/catch | 미들웨어 또는 withAuth 래퍼 |
| `console.error`와 Sentry 혼재 | 일부 에러만 Sentry 전송 | 통일된 에러 로거 |
| 레거시 API 라우트 | `/api/ai/titles`, `/api/ai/tags` 미사용 | 제거 |
...
| 유저 DB (히스토리/즐겨찾기) | 🔥 유료화 시 필수 | Supabase |
| 모바일 반응형 | 높음 | Tailwind 브레이크포인트 |
| 배경제거 Replicate BRIA | 중간 | 품질+비용 개선 |
| 태그(라벨) 제거 (인페인팅) | 중간 | Gemini 프롬프트 or Replicate |
...
| 100명 × 40% 사용률 | ~22만원 |
| 150명 × 40% 사용률 | ~33만원 |
| 150명 × 100% 사용률 (최악) | ~82만원 |

**Google Cloud 예산 한도 월 50만원 설정 상태**. Replicate BRIA 도입 시 배경제거 비용 1/2로 감소.
